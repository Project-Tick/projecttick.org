...
[`sccache`](https://github.com/mozilla/sccache) is a compiler cache. It speeds up recompilation by caching previous compilations and detecting when the same compilation is being done again.

You can [find out how to download it here](https://github.com/mozilla/sccache?tab=readme-ov-file#installation). After setting up, builds will be incremental, and the builds after the first one will be much faster.
...
4. Set `cppStandard` to `c++17` or higher.
	...
	"configurations": [
		{
			...
			"compilerArgs": ["-L/opt/homebrew/opt/qt@6/lib"],
